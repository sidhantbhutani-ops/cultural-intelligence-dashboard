...
(async () => {
  try {
    console.log(`[Seed] Inserting ${sources.length} sources...`);
    for (const source of sources) {
      await query(
        'INSERT INTO scraper_sources (name, source_type, base_url, scrape_strategy, engagement_metric, rate_limit_per_hour, priority, description, is_active) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)',
        source
      );
    }
    const result = await query('SELECT COUNT(*) as count FROM scraper_sources');
    console.log(`[Seed] ✅ Complete. Total sources: ${result.rows[0].count}`);
    process.exit(0);
  } catch (err) {
