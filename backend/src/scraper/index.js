...
class ScraperOrchestrator {
  ...
  async run(providedRunId) {
    ...
    try {
      ...
      const scrapedSourceNames = [];
      const promises = sources.map(source =>
        this.fetchSourceContent(source)
          .then(content => { scrapedSourceNames.push(source.name); return content; })
          .catch(err => {
      ...
      await this.logRun(runId, 'completed', allItems.length, storedTrends.length, uniqueItems.length - storedTrends.length, null, duration, scrapedSourceNames, topicsFound);
  ...
  async logRun(runId, status, itemsFetched, trendsCreated, trendsSkipped, errorMessage, durationMs, scrapedSourceNames, topicsFound) {
    try {
      await query(
        ...
        [runId, status, itemsFetched, trendsCreated, trendsSkipped, errorMessage, Math.ceil(durationMs / 1000), scrapedSourceNames, 'api', topicsFound]
      );
    } catch (err) {
